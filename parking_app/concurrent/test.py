...
def consume(waitC, waitP, listaNumeri, lock):
    lock.acquire()
...
def produce(waitC, waitP, listaNumeri, lock):
    lock.acquire()
    if (listaNumeri.getLen() > 0):
        waitP.wait()
    for i in range(10):
...
def main():
    ...
    producer = Process(target = produce, args =(waitC, waitP, listaNumeri,lock,))
    producer.start()
    time.sleep(2)
    consumer = Process(target = consume, args =(waitC, waitP, listaNumeri,lock,))
    consumer.start()
    producer.join()
    consumer.join()
main()